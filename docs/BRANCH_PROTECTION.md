...
### Q2: 为什么我设置了分支保护规则，管理员还是可以直接推送？

**A:** 这是因为没有勾选以下两个关键选项：
1. ❌ **Include administrators**（包括管理员）- 如果没有勾选，管理员可以绕过所有保护规则
2. ❌ **Do not allow bypassing the above settings**（不允许绕过上述设置）- 如果没有勾选，管理员可以绕过保护规则
...
配置这两个选项后，即使是管理员也必须通过 PR 流程才能合并代码。
...
1. 进入 Settings → Collaborators（合作者）
2. 在 **Collaborators** 列表中查看所有合作者及其权限
3. 权限类型包括：
   - **Read**（读取）- 只能查看和克隆代码
   - **Triage**（分类）- 可以管理 issue 和 PR，但不能推送代码
   - **Write**（写入）- 可以推送代码，创建分支和 PR
   - **Maintain**（维护）- 可以管理仓库设置，但不能删除仓库
   - **Admin**（管理员）- 完全访问权限，包括删除仓库

**注意：** 只有仓库所有者（Owner）才能查看和修改合作者权限。
